...
import TraderPopupForm from './TraderPopupForm';
...
const Bid: React.FC = () => {
  ...
  //         setMessage("Trader minimum balance should be "+res.data.content.minimumMarketBalance);
  //         setIserror(true)
  ...
  //     .catch(error => {
  //       setIsMinimumBalanceValid(false)
  //       setMessage("Failed to check Trader balance");
  //       setIserror(true)
  //       setButtonDisabled(false)
  //     })
  // }
  ...
  const reelerTransactionReport = () => {
    const reelerTransactionReportData = {
        ...
        "traderLicenseId":  parseInt(localStorage.getItem("userTypeId")!),
    }
...
//     };
   ...
   if(radiusString != null && radiusString != "" && radiusString != "null" && radiusString != undefined){
    ...
    if(!isinrange){
      setIsUserInExactLocation(false)
      setMessage("Your location is far away from market"+location.coords.latitude+":Lang"+location.coords.longitude);
      setIserror(true)
      setButtonDisabled(true)
    }else{
...
//Trader API Added
  ...
  const handleBidBtn = () => {
    ...
    api.post("market-auction/v1/auction/trader/submitBid", submitBidData, {
      headers: {
        "Content-Type": "application/json",
        accept: "*/*",
        Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
  ...
  const fetchHighestBidForLot = (_lotid: any) => {
    ...
    api.post("market-auction/v1/auction/trader/getHighestBidPerLot", highestBidData, {
      headers: {
        "Content-Type": "application/json",
        accept: "*/*",
        Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
  ...
  const handleRefreshBtn = () => {
    ...
    api.post("market-auction/v1/auction/trader/getHighestAndCurrentBidByEachLotForTrader", submitData, {
      headers: {
        "Content-Type": "application/json",
        accept: "*/*",
        Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
